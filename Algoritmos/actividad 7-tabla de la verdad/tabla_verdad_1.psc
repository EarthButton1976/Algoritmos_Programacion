...
Algoritmo tabla_verdad_1
	Definir p, q Como Logico
	Definir p_and_q, not_q, resultado Como Logico
	...
	Escribir "Tabla de la verdad 1: (p AND q) OR (NOT q)"
	...
	Para fila <- 1 Hasta 4 Con Paso 1 Hacer
		...
		Escribir "  NOT q: "
		Leer r2
		Escribir "  (p AND q) OR (NOT q): "
		Leer r3
		
		Si (r1 = "V" O r1 = "v") = p_and_q Entonces
			aciertos <- aciertos + 1
		SiNo
			errores <- errores + 1
		FinSi
		
		Si (r2 = "V" O r2 = "v") = not_q Entonces
			aciertos <- aciertos + 1
		SiNo
			errores <- errores + 1
		FinSi
